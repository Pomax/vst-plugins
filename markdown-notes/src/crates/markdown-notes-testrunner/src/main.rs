...
//! against it.
//!
//! Usage: `markdown-notes-testrunner [path-to-plugin-binary]`. With no argument it
...
fn plugin_file_name() -> &'static str {
    ...
    {
        "markdown_notes_plugin.dll"
    }
    #[cfg(target_os = "macos")]
    {
        "libmarkdown_notes_plugin.dylib"
    }
    #[cfg(target_os = "linux")]
    {
        "libmarkdown_notes_plugin.so"
    }
}
...
///
/// `markdown-notes-testrunner` deliberately does not depend on `markdown-notes-plugin` — it
...
fn warn_if_stale(plugin: &std::path::Path) -> bool {
    ...
    // This crate lives at <root>/src/crates/markdown-notes-testrunner.
    ...
    let newest = [
        "src/crates/markdown-notes-plugin/src",
        "src/crates/markdown-notes-core/src",
    ]
        .iter()
        ...
        Some(source) if source > built => {
            eprintln!(
                "stale plugin binary: {} is older than the sources it was built from.\n\
                 Run `cargo build -p markdown-notes-plugin` (or `cargo run -p xtask -- test`) first —\n\
