...
//! note-taking plugin it is the honest shape: the document is edited in the GUI
//! and saved by the processor, and one object means one document with no
//! marshalling.
//!
...
use markdown_notes_core::{Command, Editor, DEFAULT_HEIGHT, DEFAULT_WIDTH, MIN_HEIGHT, MIN_WIDTH};
...
impl IPluginBaseTrait for MarkdownNotes {
...
impl IComponentTrait for MarkdownNotes {
...
impl IAudioProcessorTrait for MarkdownNotes {
...
impl IProcessContextRequirementsTrait for MarkdownNotes {
    unsafe fn getProcessContextRequirements(&self) -> u32 {
        0
    }
}

impl IEditControllerTrait for MarkdownNotes {
    ...
    unsafe fn createView(&self, name: *const c_char) -> *mut IPlugView {
        ...
        ComWrapper::new(MarkdownNotesView::new(self.editor.clone()))
...
impl MarkdownNotesView {
    pub fn new(editor: Shared) -> MarkdownNotesView {
        MarkdownNotesView {
            editor,
...
impl Class for MarkdownNotesView {
    type Interfaces = (IPlugView,);
}

impl IPlugViewTrait for MarkdownNotesView {
...
impl ClassKind {
    ...
    fn cid(self) -> TUID {
        match self {
            ClassKind::Processor => MarkdownNotes::CID,
        }
    }
...
impl IPluginFactoryTrait for Factory {
    unsafe fn getFactoryInfo(&self, info: *mut PFactoryInfo) -> tresult {
        ...
        copy_cstring("https://example.invalid/markdown-notes", &mut info.url);
    ...
    unsafe fn createInstance(
        ...
        if *(cid as *const TUID) != MarkdownNotes::CID {
            return kInvalidArgument;
        }
        let Some(instance) = ComWrapper::new(MarkdownNotes::new()).to_com_ptr::<FUnknown>() else {
            return kInternalError;
        };
