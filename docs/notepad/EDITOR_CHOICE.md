...
The notepad has to do two things at once, and until now it did one at a time.
...
selection, undo and key handling are all hand-written in `notepad-core`, and
...
- `notepad-core::Editor` keeps its public shape and hands the work to
  `kode_markdown::MarkdownEditor`. One per tab, so the document lives in one
...
- The renderer, the tab strip, the colours, the state and the file handling
  stay as they are.
