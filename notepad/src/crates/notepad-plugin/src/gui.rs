...
/// How much larger the notepad's name is than the toolbar's buttons.
const TITLE_SIZE_BUMP: f32 = 3.0;

/// Shown in the name field while it is empty.
const TITLE_PLACEHOLDER: &str = notepad_core::DEFAULT_TITLE;

/// How far a code block reaches above and below the rows it holds.
const CODE_PADDING: f32 = 5.0;

/// Breathing room around the tab strip.
const TAB_MARGIN: egui::Margin = egui::Margin::symmetric(10, 4);

/// How wide a tab is: whatever this string measures in the button font. It is
/// the same string [`notepad_core::MAX_TITLE_CHARS`] counts, so a title that
/// fits the character limit fits the tab.
const TAB_WIDTH_SAMPLE: &str = "this many words";
...
struct Gui {
    ...
    applied_colours: Option<notepad_core::Colours>,
    ...
    /// The notepad's name while the field has focus. The editor is only
    /// updated when it changes, and the field is only refreshed from the
    /// editor while it is not being typed into.
    title: String,
    /// Tab being dragged, and where each tab was drawn last frame, which is
    /// what a drop position is worked out against.
...
fn draw_ui(ui: &mut egui::Ui, gui: &mut Gui) -> Color32 {
    ...
    // only when the document is what was clicked into. The notepad's name and
...
fn apply_theme(ui: &mut egui::Ui, gui: &mut Gui) -> Color32 {
    let (theme, colours) = match gui.editor.lock() {
        ...
        Err(_) => (Theme::Auto, notepad_core::ColourScheme::default()),
    };
...
fn rgba(c: notepad_core::Rgba) -> Color32 {
...
fn paint_visuals(visuals: &mut egui::Visuals, c: &notepad_core::Colours, dark: bool) {
    ...
    visuals.selection.stroke.color = rgba(c.tab_selected_label);
    ...
    // The selected tab is drawn as a "selected" button.
    widgets.open.weak_bg_fill = rgba(c.tab_selected_fill);
    widgets.open.fg_stroke.color = rgba(c.tab_selected_label);
}
...
fn cog_button(ui: &mut egui::Ui, gui: &Gui) -> egui::Response {
    ...
    let colours = match gui.editor.lock() {
        ...
        Err(_) => *notepad_core::ColourScheme::default().for_mode(ui.visuals().dark_mode),
    };
...
fn document_id() -> egui::Id {
    egui::Id::new("notepad-document")
}

/// The notepad's own name, editable in place.
fn title_field(ui: &mut egui::Ui, gui: &mut Gui) {
    ...
    let response = ui.add(
        egui::TextEdit::singleline(&mut gui.title)
            ...
            .font(egui::FontId { size: font.size + TITLE_SIZE_BUMP, ..font })
            .text_color(bold)
            .hint_text(TITLE_PLACEHOLDER),
    );
    ...
    // An empty name is a name nobody can see, so it reverts on the way out.
    if response.lost_focus() && gui.title.trim().is_empty() {
        gui.title = notepad_core::DEFAULT_TITLE.to_string();
        if let Ok(mut e) = gui.editor.lock() {
            e.title = gui.title.clone();
        }
    }
...
/// The tab strip: one tab per section of the document.
///
/// Tabs are the same document seen in parts, so there is no "unsaved tab" —
/// closing one removes that section from what gets saved.
fn tabs(ui: &mut egui::Ui, gui: &mut Gui) {
    let (count, active, titles) = match gui.editor.lock() {
        Ok(e) => {
            let titles: Vec<(String, String, bool)> = (0..e.tab_count())
                .map(|i| (e.tab_display_title(i), e.tab_title(i), e.tab_title_is_truncated(i)))
                .collect();
            (e.tab_count(), e.active_tab(), titles)
        }
        Err(_) => return,
    };

    // Every tab is the same width — that of the longest title allowed — so the
    // strip does not shuffle sideways while a heading is being typed.
    let tab_size = fixed_width(ui, &[TAB_WIDTH_SAMPLE]);
    let tab_width = tab_size.x;
            ...
            let response = ui
                .add(
                    egui::Button::new(shown.as_str())
                        .selected(selected)
                        .min_size(egui::Vec2::new(tab_width, 0.0))
                        // A title of wide glyphs can still overrun the limit
                        // in pixels; clipping keeps every tab the same size.
                        .truncate()
    ...
    // A drag ends over whichever tab the pointer is on; that is where it lands.
    ...
    // back from whatever field had it. Without this, adding a tab after
    // renaming the notepad leaves the new tab unable to receive anything.
...
type Field = (&'static str, fn(&mut notepad_core::Colours) -> &mut notepad_core::Rgba);

const CHROME_FIELDS: &[Field] = &[
    ("Window background", |c| &mut c.window_background),
    ("Toolbar and tabs", |c| &mut c.bar_fill),
    ...
    ("Selected tab", |c| &mut c.tab_selected_fill),
    ("Selected tab label", |c| &mut c.tab_selected_label),
                ...
                for (name, field) in fields {
                    ui.horizontal(|ui| {
                        ...
                        if ui.color_edit_button_srgba(&mut value).changed() {
                            ...
                            *colour = notepad_core::Rgba::rgba(r, g, b, a);
                            changed = true;
                        }
                        ui.label(*name);
                    });
...
impl Palette {
    fn from(c: &notepad_core::Colours) -> Palette {
        Palette {
