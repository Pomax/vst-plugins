//! Tab titles, and the split between one document and the tabs holding it.
//!
//! The tabs are a view of a single document, not separate files. Saving joins
//! them back together in tab order; loading splits the result again on its
//! top-level headings, so a file written by the editor reopens with the tabs
//! it was written with.

/// Shown by a tab whose text does not start with a heading.
pub const UNTITLED: &str = "untitled";

/// Widest a tab title may be, in characters.
///
...
/// A tab's title: the heading it opens with, or [`UNTITLED`].
...
/// that never uses `#` opens as a single tab. Always returns at least one
...
/// Join tabs back into one document, in tab order.
...
mod tests {
    ...
    #[test]
    fn a_tab_without_a_heading_is_untitled() {
    ...
    #[test]
    fn an_empty_document_is_one_empty_tab() {
        assert_eq!(split_document(""), vec![String::new()]);
    }

    #[test]
    fn a_document_without_headings_is_one_tab() {
        assert_eq!(split_document("no headings here"), vec!["no headings here"]);
    }

    #[test]
    fn each_top_level_heading_starts_a_tab() {
        let parts = split_document("# One\n\nfirst\n\n# Two\n\nsecond");
        assert_eq!(parts, vec!["# One\n\nfirst", "# Two\n\nsecond"]);
    }

    #[test]
    fn text_before_the_first_heading_is_its_own_tab() {
    ...
    #[test]
    fn tabs_join_in_order() {
        let joined = join_document(&["# One\n\nfirst", "# Two\n\nsecond"]);
        assert_eq!(joined, "# One\n\nfirst\n\n# Two\n\nsecond");
    }

    #[test]
    fn empty_tabs_contribute_nothing_to_the_document() {
        assert_eq!(join_document(&["# One", "", "# Two"]), "# One\n\n# Two");
    }

    #[test]
    fn headed_tabs_survive_a_save_and_load() {
        let tabs = ["# One\n\nfirst", "# Two\n\nsecond", "# Three\n\nthird"];
        let reopened = split_document(&join_document(&tabs));
        assert_eq!(reopened, tabs);
    }
}
